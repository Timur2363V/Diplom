...
namespace dpn
{
    ...
    {
        public static DpnStandaloneInputModule Instance { get; protected set; }
        ...
        new protected static PointerEventData.FramePressState StateForMouseButton(int buttonId)
        {
            ...
            {
                // flip controller
                if(DpnCameraRig._instance.CurrentPeripheral.PeripheralType == DPVRPeripheral.Flip)
                {
                ...
                // nolo controller
                if(DpnCameraRig._instance.CurrentPeripheral.PeripheralType == DPVRPeripheral.Nolo)
                {
        ...
        {
            if (DpnCameraRig._instance.CurrentPeripheral == null)
                return;
        ...
        Vector2 GetPointerScreenPosition()
        {
            ...
            {
                Ray ray = DpnCameraRig._instance.CurrentPeripheral.GetRay();

                Vector3 pos = ray.origin + ray.direction * 10000000.0f;
                position = DpnCameraRig._instance._center_eye.WorldToScreenPoint(pos);
            }
            else
        ...
        Vector2 GetScrollDelta()
        {
            ...
            var type = DpnCameraRig._instance.CurrentPeripheral.PeripheralType;
            switch (type)
            {
