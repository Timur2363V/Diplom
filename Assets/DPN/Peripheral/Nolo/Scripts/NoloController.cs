...
# else
        ...
        {
            return _currButtonState;
        }

        public bool ButtonDown(NoloButtonMask buttonMask)
        {
            int mask = (int)buttonMask;
            return (_prevButtonState & mask) != mask
                && (_currButtonState & mask) == mask;
        }

        public bool ButtonUp(NoloButtonMask buttonMask)
        {
            int mask = (int)buttonMask;
            return (_prevButtonState & mask) == mask
                && (_currButtonState & mask) != mask;
